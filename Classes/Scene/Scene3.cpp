...
bool Scene3::init()
{
    ...
    auto closeItem = MenuItemImage::create("CloseNormal.png","CloseSelected.png",CC_CALLBACK_1(Scene3::menucloseCallBack, this));//退出按钮
    ...
    */
    ...
    this->schedule(schedule_selector(Scene3::controlMoveArea), 0.1f);//控制外围边界
...
void Scene3::menucloseCallBack(Ref* pSender)//关闭按钮的回调
{
...
bool Scene3::onContactBegan(cocos2d::PhysicsContact & contact)//碰撞的回调
{
...
bool  Scene3::onKeyPressed(cocos2d::EventKeyboard::KeyCode keycode, cocos2d::Event *event)//按键按下的回调
{
...
void Scene3::monsterinit()//怪物初始化
{
    
    //this->scheduleOnce(schedule_selector(Scene3::addmonster), 0.1f);//在0.1s后添加怪物
    this->addmonster(1);
    srand((unsigned int)time(nullptr));
    this->schedule(schedule_selector(Scene3::automoveM), 1.0f);//每隔一秒怪物运动d
    this->schedule(schedule_selector(Scene3::autoshootM), 1.5f);//每隔一秒怪物射击
    
}

void Scene3::addmonster(float dt)//创建并添加怪物
{
...
void Scene3::autoshootM(float dt)//自动攻击
{
